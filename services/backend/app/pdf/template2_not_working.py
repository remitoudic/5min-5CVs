...
class TemplateCv2:
    def __init__(self, buffer):
        ...
        font_dir = '/Users/remus/Documents/CV_builder/services/backend/app/pdf/styles/fonts'  # or whatever directory you put the fonts in
        ...
        self.styles.add(ParagraphStyle(name='Name', fontName='Roboto-Bold', fontSize=24, textColor=colors.HexColor("#2C3E50"), spaceAfter=12))
        self.styles.add(ParagraphStyle(name='Section', fontName='Roboto-Bold', fontSize=14, textColor=colors.HexColor("#2980B9"), spaceBefore=12, spaceAfter=6))
        self.styles.add(ParagraphStyle(name='Normal', fontName='Roboto', fontSize=10, textColor=colors.HexColor("#34495E"), spaceAfter=6))
        self.styles.add(ParagraphStyle(name='Right', fontName='Roboto', fontSize=10, textColor=colors.HexColor("#34495E"), alignment=TA_RIGHT))
    ...
    def add_career_objectives(self, objectives):
        ...
        self.flowables.append(Paragraph(objectives, self.styles['Normal']))

    def add_work_experience(self, work_experience):
        self.flowables.append(Paragraph("Work Experience", self.styles['Section']))
        for exp in work_experience:
            data = [
                [Paragraph(exp["position"], self.styles['Normal']), Paragraph(exp["date"], self.styles['Right'])],
                [Paragraph(exp["company"], self.styles['Normal']), ""]
            ]
    ...
    def add_education(self, education):
        ...
        for edu in education:
            data = [
                [Paragraph(edu["school"], self.styles['Normal']), Paragraph(edu["date"], self.styles['Right'])]
            ]
            table = Table(data, colWidths=[350, 150])
            table.setStyle(TableStyle([
                ('VALIGN', (0, 0), (-1, -1), 'TOP'),
